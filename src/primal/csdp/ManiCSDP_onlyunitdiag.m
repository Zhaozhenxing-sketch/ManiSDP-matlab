% This function solves CSDPs with unit-diagonal and without extra affine constraints using the primal approach:
% Min  <C, X>
% s.t. X >= 0
%      diag(X) = 1

function [X, obj, data] = ManiCSDP_onlyunitdiag(C, options)
...
for iter = 1:options.AL_maxiter
    ...
    X = Y'*Y;
    z   = real(sum(conj(X).*C, 1));
    obj = real(sum(conj(C(:)).*X(:)));
    S = C - diag(z);
...
fprintf('ManiCSDP: optimum = %0.8f, time = %0.2fs\n', obj, toc(timespend));
    ...
    function [H, store] = hess(Y, U, store)
        eH = U*C;
        H = eH - Y.*real(sum(conj(Y).*eH)) - U.*eG;
    end
