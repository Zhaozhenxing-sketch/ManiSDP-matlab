function [A, b, c, K] = bqpmom_complex_nounitdiag(Q, e)
    ...
    % 1. 显式处理常数项类 (净指标为 0 的类)，设定基准 X(1,1) = 1
      ...
      if isKey(class_map, zero_label)
        ...
        % 让零类中非对角位置等于 X(1,1)=1 (对角元素由流形保证，跳过)
        base_idx = zero_idxs(1);
        for m = 2:length(zero_idxs)
            idx_m = zero_idxs(m);
            if mod(idx_m - 1, N) == floor((idx_m - 1) / N), continue; end
            [Ar, Ai] = get_diff_matrices(base_idx, idx_m, N);
